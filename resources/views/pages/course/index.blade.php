                                ...
                                <template x-for="(course, index) in paginatedData" :key="course.id">
                                    <tr class="border-t border-gray-100 dark:border-white/[0.5]">
                                        ...
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-right">
                                            <p class="text-theme-sm text-gray-700 dark:text-gray-400" x-text="course.price"></p>
                                        </td>
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-right">
                                            <p class="text-theme-sm text-gray-700 dark:text-gray-400" x-text="course.offline_price"></p>
                                        </td>
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-right">
                                            <p class="text-theme-sm text-gray-700 dark:text-gray-400" x-text="course.non_regular_price"></p>
                                        </td>
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-right">
                                            <p class="text-theme-sm text-gray-700 dark:text-gray-400" x-text="course.private_price"></p>
                                        </td>
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-center">
                                        ...
                                        <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05] text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <button @click="deleteRow(course.id,course.name)" class="text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-500" title="Hapus">
                                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('courseTable', () => ({
            ...
            init() {
                ...
                // Event load data sata proses simpan dan update data
                window.addEventListener('course-data-updated', () => {
                    this.loadData();
                });
            },
            ...
            async loadData() {
                this.isLoading = true;
                try {
                    const response           = await fetch(`{{ route('courses.data') }}?search=${this.search}&status=${this.statusFilter}&sort_column=${this.sortColumn}&sort_direction=${this.sortDirection}&per_page=${this.perPage}&page=${this.currentPage}`);
            ...
            confirmDelete() {
                ...
                fetch(`/admin/courses/${this.deleteModal.id}`, {
                    method: 'DELETE',
                    headers: {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#tax').val(course.tax);
                    $('#offline_price').val(formatNumberDisplay(course.offline_price));
                    $('#offline_discount').val(formatNumberDisplay(course.offline_discount));
                    $('#offline_tax').val(formatNumberDisplay(course.offline_tax));
                    $('#non_regular_price').val(formatNumberDisplay(course.non_regular_price));
                    $('#non_regular_discount').val(formatNumberDisplay(course.non_regular_discount));
                    $('#non_regular_tax').val(formatNumberDisplay(course.non_regular_tax));
                    $('#private_price').val(formatNumberDisplay(course.private_price));
                    $('#private_discount').val(formatNumberDisplay(course.private_discount));
                    $('#private_tax').val(formatNumberDisplay(course.private_tax));
